...
void Gpt<T>::forwardDecoder(
	...
	const T* cur_layer_input = current_layer_input;
	const GptLayerWeight<T>& cur_layer_weight = weight.layer_weights[layer_id];

	// st::kernel::setIntermediateResult<T>(
	// 	from,
	// 	to,
	// 	num_tokens,
	// 	hidden_size
	...
	if (operation == 2) {
		if (hyper_param.is_attn_out_biased) {
			kernel::addbiasBatched(
				attention_out.ptr,
				d_input,
				cur_layer_weight.attn_out_bias,
				num_tokens,
				hyper_param.hidden_size
			);
			sync_check_cuda_error();
	...
	if (operation == 3) {
		if (!hyper_param.is_gated_ffn) {
			kernel::addbiasBatched(d_output, d_input, cur_layer_weight.ffn_fc2_bias, num_tokens, hyper_param.hidden_size);
			sync_check_cuda_error();
...
std::vector<int64_t> Gpt<T>::forward(
	...
	if (parallelism_param.is_first_stage() && operation == 0) {
		...
		// 	// );
		// 	// for (int i = 0; i < num_tokens * this->hyper_param.hidden_size; ++i) {
		// 	// 	d_decoder_input.ptr[i] = ((T*)intermed_input)[i];
		// 	// }
		// }
	...
	if (parallelism_param.is_last_stage() && operation == -1){
		// Layernorm / RMSNorm
		if (hyper_param.is_rmsnorm) {
			st::kernel::rmsnorm<T>(
				d_decoder_output.ptr,
		...
		// 	// for (int i = 0; i < num_tokens * this->hyper_param.hidden_size; ++i) {
		// 	// 	((T*)intermed_output)[i] = d_decoder_output.ptr[i];
		// 	// }
