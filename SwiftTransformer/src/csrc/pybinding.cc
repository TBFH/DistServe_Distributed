...
std::vector<int64_t> forwardWrapper(const c10::intrusive_ptr<T>& self,
                                    ...
                                    torch::Tensor &intermed_input,
                                    torch::Tensor &intermed_output,
                                    torch::Tensor &current_layer_input,
                                    const int64_t layer_id,
                                    const int64_t operation) {
  return self->forward(input_tokens_batched, first_token_indexes, k_cache, v_cache, block_table, intermed_input, intermed_output, current_layer_input, layer_id, operation);
}
